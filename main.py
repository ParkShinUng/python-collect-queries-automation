...
async def main():
    ...
    async with async_playwright() as p:
        ...
        if await page0.locator('button[data-testid="login-button"]').count() > 0 == False:
            await page0.locator('button[data-testid="login-button"]').click()
            await page0.locator('input[id="email"]').fill(cfg.USER_ID)
            await page0.locator('button[type="submit"]').click()
            await page0.locator('input[name="current-password"]').fill(cfg.USER_PW)
            await page0.locator('button[type="submit"]').click()
            await page0.goto(cfg.chatgpt_url, wait_until="load")
        ...
        for sheet_name in excel_mgr.sheet_list:
            ...
            log(f"\n{sheet_name}Sheet 모든 작업 완료. 결과가 '{cfg.excel_path}'에 저장되었습니다.")
